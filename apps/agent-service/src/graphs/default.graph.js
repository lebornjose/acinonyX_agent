 ...
 * @returns {{ stockAnalysis: object }} 行情分析结果
 */
async function stockAnalysisNode(state, stockTool) {
  const rawTask = state.rawTask || state.task;
  return {
    stockAnalysis: await stockTool.invoke({ task: rawTask })
  };
}
 ...
 */
export function createDefaultGraph({ researcher, writer, stockTool, checkpointer }) {
